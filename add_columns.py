...
def fixed_baselines(dataframe):
  ...
  while (longest_day >= 0):
    ...
    if len(day_past_forecast['pred']) >= 1:
        ...
        day_counter+=1
    longest_day -=1
# if day_counter is not bigger than 0, it is returning nothing
# why do i need day_counter to be bigger than 0?
# question 2082
  if day_counter > 0:
...
def new_all_questions_baseline(id_file, path):
  ...
  with open(id_file, "r") as f:
    for line in f.readlines():
      ...
      id_counter +=1
      print(id_counter, line)
      # each_baseline is returning NoneType
